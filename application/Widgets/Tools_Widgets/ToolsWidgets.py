...
class ToolsWidgets(customtkinter.CTkFrame):
    def __init__(self,root):
        ...
        self._sliders = []
        self._value_labels = []  # スライダーの値表示用のラベルを管理
    ...
    # slider_valuesのvalueを受け取りsliderに適応、値ラベルと共に描画
    def draw_sliders(self, slider_values: dict):
        ...
        for i, (color, val) in enumerate(slider_values.items()):
            ...
            slider = customtkinter.CTkSlider(
                ...
                command=lambda value, c=color, v=val: self.update_label(c, v),
            )
            ...
            label.place(x=x + padx * i-10, y=y + 120)  # スライダーの下に配置
    ...
    def reset_slider_value(self,color):
        ...
        slider_value.set(0)
        self.update_label(color,slider_value)

    # スライダーの値が変わるたびにラベルを更新
    def update_label(self, color, variable):
        for label in self._value_labels:
