...
class ImageWidgets(customtkinter.CTkFrame):  
    def __init__(self, root: TkinterDnD.Tk):
        ...
        self.label = customtkinter.CTkLabel(root, text="ここに画像をD&D", text_color="white", fg_color="#000000", font=(None,30))  # LabelをCTkLabelに変更
        self.image_label = customtkinter.CTkLabel(self,text="")
        self.label.place(relx=0.37, rely=0.37, anchor="center")

        # tkinterのwrapperであるtkinterdnd2で提供されているメソッドを使用してD&Dを有効化。トリガーをセット
        self.drop_target_register(DND_FILES)
        self.dnd_bind('<<Drop>>', self.drop)
        
        self.pack_propagate(False)

    # drop時処理: path取得→ open → resize → labelで表示(初回でD&Dlabel削除)
